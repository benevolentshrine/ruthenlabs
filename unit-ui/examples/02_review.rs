    ...
    loop {
        ...
        terminal.draw(|f| {
            ...
            let [top, main, side, bot] = Layout::vertical([
                Constraint::Length(1), Constraint::Min(3), Constraint::Length(8), Constraint::Length(1),
            ]).areas(area);

            let chunks = Layout::horizontal([Constraint::Percentage(60), Constraint::Percentage(40)]).split(main);
            let diff_area = chunks[0];
            let tl_area = if area.width > 80 { chunks[1] } else { Rect::default() };

            let bar = StatusBar::new().provider("Review Bot")
            ...
            f.render_widget(DiffView::new().diff(diff_text).file("src/checkout.rs").style(s.clone()), inner);
            ...
            let entries = [
                ...
                TimelineEntry::new("10:32:20", if approved { "✓" } else { "◌" }, if approved { "approved" } else { "awaiting_approval" }, ""),
            ];
            ...
            let st = format!(" {} | arrows: toggle panels | Enter: approve | q: quit",
                if approved { "✓ Approved" } else { "◌ Pending" });
            f.render_widget(Paragraph::new(st).style(Style::default().fg(s.text_dim)), bot);
        })?;
