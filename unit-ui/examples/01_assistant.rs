...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut mode = 0u16;
    let msgs = [
        ("Hi! I can help you code. What are you working on?", Role::Assistant),
    ...
    loop {
        ...
        terminal.draw(|f| {
            ...
            let [top, chat, tool, status, bot] = Layout::vertical([
                Constraint::Length(1), Constraint::Min(3), Constraint::Length(5),
                Constraint::Length(3), Constraint::Length(3),
            ]).areas(area);

            let bar = StatusBar::new().provider("Unit AI").model("assistant-v1")
                .connection(ConnectionStatus::Connected).started_at(started)
                .token_count(142).style(s.clone());
            ...
            if mode == 0 {
                let tb = ThinkingBlock::new().label("Reasoning about auth module")
                    .state(ThinkingState::Thinking).expanded(true).frame_index(frame as usize)
                    .elapsed(Duration::from_millis(frame * 50)).style(s.clone());
                f.render_widget(&tb, tool);
            } else {
                let card = ToolCallCard::new().tool_name("search_files")
                    .arguments("{\"pattern\": \"auth*\", \"path\": \"src/\"}")
                    .status(if mode < 5 { ToolStatus::Running } else { ToolStatus::Success })
                    .duration(Duration::from_millis(mode as u64 * 100)).style(s.clone());
                f.render_widget(&card, status);

                let card2 = ToolCallCard::new().tool_name("read_file")
                    .arguments("{\"path\": \"src/auth/login.rs\"}")
                    .status(ToolStatus::Pending).style(s.clone());
                f.render_widget(&card2, tool);
            }

            let input = BasicInput::new(&input_text).placeholder("Ask or type a command...").style(s.clone());
        ...
        if crossterm::event::poll(std::time::Duration::from_millis(80))? {
            if let crossterm::event::Event::Key(key) = crossterm::event::read()? {
                match key.code {
                    ...
                    crossterm::event::KeyCode::Enter => { mode += 1; if mode > 8 { mode = 0; } }
