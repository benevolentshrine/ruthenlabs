...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut messages: Vec<(&str, Role)> = vec![
        ("Welcome to session: Chat #1", Role::Assistant),
    ];
    ...
    loop {
        terminal.draw(|f| {
            ...
            let bar = StatusBar::new()
                .provider("Sessions").model("manager")
                .connection(ConnectionStatus::Connected).started_at(started)
                .token_count(messages.len() as u64).style(s.clone());
            ...
            for (i, name) in sessions.iter().enumerate() {
                ...
                let fg = if i == selected_session { s.accent } else { s.text_dim };
                let marker = if i == selected_session { "▶ " } else { "  " };
                let line = Paragraph::new(format!("{}{}", marker, name)).style(Style::default().fg(fg));
                f.render_widget(&line, Rect { x: sbi.x + 1, y, width: sbi.width.saturating_sub(2), height: 1 });
            }
        ...
        if crossterm::event::poll(std::time::Duration::from_millis(100))? {
            if let crossterm::event::Event::Key(key) = crossterm::event::read()? {
                match key.code {
                    ...
                    crossterm::event::KeyCode::Up => selected_session = selected_session.saturating_sub(1),
                    crossterm::event::KeyCode::Down => selected_session = (selected_session + 1).min(sessions.len() - 1),
                    crossterm::event::KeyCode::Enter => {
                        if !input_text.is_empty() {
                            let msg = std::mem::take(&mut input_text);
                            messages.push((Box::leak(msg.into_boxed_str()), Role::User));
                            let reply = format!("Response in {}: acknowledged.", sessions[selected_session]);
