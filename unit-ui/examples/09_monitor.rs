...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut frame = 0u64;
    let services = [("api", "healthy"), ("db", "degraded"), ("cache", "healthy"), ("queue", "healthy")];
    let entries = [
    ...
    loop {
        ...
        terminal.draw(|f| {
            ...
            let [top, services_area, card_area, tl_area] = Layout::vertical([
                Constraint::Length(1), Constraint::Length(3), Constraint::Length(4), Constraint::Min(2),
            ]).areas(area);

            let bar = StatusBar::new().provider("System Monitor")
            ...
            for (i, (area, (name, status))) in panels.iter().zip(services.iter()).enumerate() {
                ...
                let color = if *status == "healthy" { s.success } else { s.thinking };
                f.render_widget(Paragraph::new(*status).style(Style::default().fg(color)), inner);

                let dot_frame = (frame as usize + i * 2) % 6;
                f.render_widget(ThinkingDots::new().frame_index(dot_frame).style(s.clone()),
                    Rect { x: inner.x, y: inner.y + 1, width: inner.width, height: 1 });
            }

            let card = ToolCallCard::new().tool_name("health_check")
                .arguments("{\"endpoints\": [\"api\", \"db\", \"cache\", \"queue\"]}")
                .status(ToolStatus::Running).duration(Duration::from_millis(frame * 20))
                .result("3/4 healthy — db degraded")
                .style(s.clone());
            f.render_widget(&card, card_area);

            f.render_widget(SessionTimeline::new().entries(&entries).style(s.clone()), tl_area);
        })?;
