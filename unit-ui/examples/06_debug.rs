    ...
    loop {
        ...
        if frame_count % 15 == 0 {
            let entry = if frame_count % 30 == 0 {
                ("[ERROR] Connection timeout on request #42", Color::Rgb(255, 80, 80))
        ...
        terminal.draw(|f| {
            ...
            let bar = StatusBar::new()
                .provider("Debug Console").model("diagnostic")
                .connection(if frame_count % 30 < 25 { ConnectionStatus::Connected } else { ConnectionStatus::Disconnected })
                .started_at(started).style(s.clone());
            ...
            let top_line = format!("Frame: {} | Entries: {} | Status: {}", frame_count, log_entries.len(),
                if frame_count % 30 < 25 { "RUNNING" } else { "RECONNECTING" });
            let header = Paragraph::new(top_line).style(Style::default().fg(s.thinking));
            f.render_widget(&header, Rect { x: inner.x + 1, y: inner.y, width: inner.width.saturating_sub(2), height: 1 });

            let spinner = Spinner::new().label("monitoring").frame_index(frame_count).style(s.clone());
            f.render_widget(&spinner, Rect { x: inner.x + 1, y: inner.y + 1, width: inner.width.saturating_sub(2), height: 1 });

            let mut y = inner.y + 3;
            for &(msg, color) in log_entries.iter().rev().take((inner.height - 3) as usize).rev() {
            ...
            let input = BasicInput::new("").placeholder("Debug command...").style(s.clone());
