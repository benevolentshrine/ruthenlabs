/**
 * CLI Argument Parser — accepts model, permission-mode, and other flags.
 *
 * Flags:
...
Options:
  --model, -m <model>        Model to use (required — check with /models at runtime)
...
Examples:
  node src/index.mjs                  Launch TUI (no model set, use /models then /model)
  node src/index.mjs -m qwen3.5:2b    Use a specific model
  node src/index.mjs --debug          Debug mode
`.trim();
}
