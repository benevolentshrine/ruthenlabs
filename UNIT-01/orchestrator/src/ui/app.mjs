...
export function App({ agentLoop, settings }) {
    ...
    const [model, setModel] = useState(agentLoop.state.model || '');
    ...
    const handleCommand = useCallback(async (text) => {
        ...
        const { response, exit: shouldExit } = await executeCommand(text, agentLoop.state);
        if (shouldExit) {
            exit();
            return;
        }
            ...
            for await (const event of agentLoop.run(text)) {
                switch (event.type) {
                    ...
                    case 'result':
                        ...
                        if (resultStr.includes('⚠️ SANDBOX')) {
                            setSandboxAlert(resultStr);
                            addMessage({ role: 'system', content: `[sandbox] ${resultStr}` });
                        }
